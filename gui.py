...
class App(ctk.CTk):
    ...
    def reload_credentials(self):
        self.env_data = load_env()
        for key in ["LINKEDIN_EMAIL", "LINKEDIN_PASSWORD", "GMAIL_ADDRESS", "GMAIL_APP_PASSWORD"]:
            if key in self.entries:
                self.entries[key].delete(0, "end")
                self.entries[key].insert(0, self.env_data.get(key, ""))
        messagebox.showinfo("Imported", "Last saved credentials have been reloaded!")
    ...
    def start_automation(self):
        ...
        for key, entry in self.entries.items():
            if not entry.get().strip():
                messagebox.showerror("Error", "Please fill all fields and credentials before running.")
                return
    ...
    def forget_user_data(self):
        ...
        if confirm:
            ...
            if os.path.exists("keywords.txt"): os.remove("keywords.txt")
